...
def resolve_path(path: Optional[str], base_dir: Path) -> Optional[str]:
    if path is None:
        return None
    p = Path(path)
    if p.is_absolute():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Infer phage-bacteria signed scores using gradients from a trained PHILM MLP_NODE model."
    )
...
def main() -> None:
    ...
    if args.input_profile is not None:
        input_paths = [resolve_path(args.input_profile, base_dir)]
    else:
        cfg_profile = get_from_config_data(config, args.split) if args.config else None
        if cfg_profile is not None and args.split != "all":
            input_paths = [resolve_path(cfg_profile, base_dir)]
        else:
            input_paths = [resolve_path(p, base_dir) for p in default_profile_paths(
                data_dir=data_dir,
                split=args.split,
    ...
    input_feature_names = resolve_path(input_feature_names, base_dir)
    output_feature_names = resolve_path(output_feature_names, base_dir)
    ...
    print(f"Input profile path(s): {', '.join(input_paths)}", file=sys.stderr)
    ...
    out_path = resolve_path(args.out, base_dir)
    write_long_table(
        raw_score=raw_score,
